...
\pic[shift={(6,0,0)}] at (grb3-east) {RightBandedCylinder={name=grb4,caption=x3,%
	...
	opacity=0.5,height=25,width={4,4,4},depth=25}};
%%
\pic[shift={(1,-6,0)}] at (grb4-east) {Cylinder={name=fv4,%
	xlabel={{"3","dummy"}},fill=\GraphConvColor,
	opacity=0.5,height=25,width={1},depth=25}};
...
\coordinate (co-split04) at ([xshift=-10pt] grb4-east);
\draw [connection] (grb4-east)
	-- (co-split04)
	-- (fv4-west -| co-split04)
	-- node {\midarrow} (fv4-west);
...
\draw [connection, -Stealth] (fv4-east) -| coordinate (mout-04) ([xshift=-40, yshift=-20] fv4-east);
...
%% Mesh template
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pic[shift={(-4,0,2)}] at (grb0-west) {Ball={name=template,%
	fill={rgb:black,5}, logo= ,%
        radius=10}};
\pic[shift={(-4,0,-2)}] at (grb0-west) {Ball={name=template-2,%
	fill={rgb:black,5}, logo= ,%
        radius=10}};
\draw [connection] ([xshift=-100] grb0-west) -- node {\midarrow} (grb0-west);
