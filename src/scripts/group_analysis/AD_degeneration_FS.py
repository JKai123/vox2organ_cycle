...
cortical thickness measurements in FS meshes. """
...
THICKNESS_DIR = "/home/fabianb/work/cortex-parcellation-using-meshes/experiments/FS/test_ADNI_CSR_large/thickness/"
...
def get_thickness_fn(file_id, structure):
    return f"{file_id}_{structure}.thickness.reg.npy"

def load_thickness_values(ids):
    th_all = []
    for i in ids:
        fn = os.path.join(THICKNESS_DIR, get_thickness_fn(i, SURFACE))
        try:
...
def ttest(specs):
    ...
    # Write
    out_file = os.path.join(OUT_DIR, f"p_thickness_{SURFACE}_FS")
